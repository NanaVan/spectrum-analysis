...
def psdWithErr_welch(bud, offset, window_length, average, overlap_ratio, padding_ratio=0, window=None, beta=None):
    '''
    Average Periodogram (Welch) Method Spectral Estimation with Error
    ...
    ***
    ...
    psd = np.mean(np.absolute(np.fft.fftshift(fft(signal), axes=-1))**2, axis=0) / np.sum(window_sequence**2) / bud.sampling_rate
    err_psd = np.std(np.absolute(np.fft.fftshift(fft(signal), axes=-1))**2, axis=0) / np.sum(window_sequence**2) / bud.sampling_rate
    return frequencies, psd, err_psd, n_dof # Hz, V^2/Hz, V^2/Hz, 1

def psdWithErr_multitaper(bud, offset, window_length, padding_ratio=0, NW=3, Kmax=4):
    '''
    Multitaper Method (MTM) with Error
    ...
    mean_psd = np.average(np.absolute(eigpsd)**2, axis=0, weights=ratio)    
    std_psd = np.sqrt(np.average((np.absolute(eigpsd)**2-mean_psd)**2, axis=0, weights=ratio))
    psd = mean_psd / bud.sampling_rate
    err_psd = std_psd / bud.sampling_rate
    return frequencies, psd, err_psd, n_dof # Hz, V^2/Hz, V^2/Hz, 1

def psdWithErr_adaptive_multitaper(bud, offset, window_length, padding_ratio=0, NW=3, Kmax=4):
    '''
    Adaptive Multitaper Method (AMTM) with Error
    ...
    signal = bud.load(window_length, offset)[1] * window_sequence
    eigpsd = np.fft.fftshift(fft(signal))
    mean_psd = np.mean(np.absolute(eigpsd[:2])**2, axis=0)
    while True:
        variance = np.sum(mean_psd) / n_point
        weight = (mean_psd / (ratio*mean_psd + (1-ratio)*variance))**2 * ratio
        mean_psd_temp = np.average(np.absolute(eigpsd)**2, axis=0, weights=weight)
        std_psd = np.sqrt(np.average((np.absolute(eigpsd)**2-mean_psd_temp)**2, axis=0, weights=weight))
        if np.allclose(mean_psd_temp, mean_psd, rtol=1e-5, atol=1e-5): break
        mean_psd = mean_psd_temp
    # number of degrees of freedom
    psd = mean_psd / bud.sampling_rate
    err_psd = std_psd / bud.sampling_rate
